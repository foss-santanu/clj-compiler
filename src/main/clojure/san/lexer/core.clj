...
(defn- check-for-error 
   ...
   (let [ [error-str rest-input] (if (and lexem input-string) 
                                     (subvec (re-find (re-pattern (str "(?s)(.*?)" lexem "(.*)"))
                                                      input-string) 1)
...
(defn- mk-token 
   ...
   (let [ lex-rules (:lexem-specs lexer)
          ...
          tk-builder (fn [{:keys [name type regex action states]}]
                         (let [ valid-state (contains? (:state updt-lexer) states) 
                                ...
                                char-count (get-in updt-lexer [:context :char-count])
                                ...
                                [lexer tk-value] (if (and (nil? lexem) (nil? rest-of-lexems))
                                                     [updt-lexer :eof]
                                                     (if (and valid-state (match? regex) fn-action)
                                                         (fn-action lexem)
                                                         [updt-lexer nil]))
                                [mod-lexer mod-input-str] (update-char-line-count lexer lexem 
                                                                                  input-string)
                                token (cond
                                         (and show-char-indx show-line-indx) {:name name :type type 
                                                                              :value tk-value 
                                                                              :char-indx char-count 
                                                                              :line-indx line-count}
                                         show-char-indx {:name name :type type :value tk-value
                                                         :char-indx char-count}
                                         show-line-indx {:name name :type type :value tk-value
                                                         :line-indx line-count}
                                         :else {:name name :type type :value tk-value})
                              ]
                              (if tk-value 
                                  (if (= tk-value :eof)
                                      [mod-lexer rest-of-lexems mod-input-str :eof]
